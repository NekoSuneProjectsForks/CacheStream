...
export const config = {
  ...
  // OAuth2 relay/broker (see apps/oauth-relay on the `oauth-relay` branch).
  // New OAuth2 platforms are brokered server-side so client secrets never
  // ship in the app. Defaults to the PUBLIC hosted instance (zero setup);
  // set OAUTH_RELAY_MODE=local + OAUTH_RELAY_URL=http://127.0.0.1:<port> to
